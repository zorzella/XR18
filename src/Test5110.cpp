...
// pin 4 - LCD chip select (CS)
// pin 3 - LCD reset (RST)
// Adafruit_PCD8544 display = Adafruit_PCD8544(MISO, 0, 2);
// Note with hardware SPI MISO and SS pins aren't used but will still be read
// and written to during SPI transfer.  Be careful sharing these pins!

// Adafruit_PCD8544 display = Adafruit_PCD8544(SCK, MOSI, MISO, 17, 16);

Adafruit_PCD8544 display = Adafruit_PCD8544(SCK, MOSI, MISO, 17, 16);
// Adafruit_PCD8544 display = Adafruit_PCD8544(5, 18, 19, 17, 16);
  ...
  while (1) {
    ...
    while (Serial.available()) {
      switch (Serial.read()) {
        ...
        case 'd':
          TRACE();
          if (display.getBias()) display.setBias(display.getBias() - 1);
          break;
        case 'R':
...
void setup_5110() {
  ...
  // rotation example
  display.clearDisplay();
  display.setRotation(1);  // rotate 90 degrees counter clockwise, can also use
